...
<script type="text/html" id="massageSaleTemp">
       <div class="wx_products_main">
		 <div class="wx_products_windows poi1" style="background-image:url({{d.content.massage_img_url}})">
              ...
              {{# } }}
			<p class="poi2 wx_products_wtxt "><span class="fl fz30 wordsbase" style="max-width:13em">{{d.content.massage_name}}</span> 
            <span class="fr fz24"><a class="wxtx_products_but1">已售{{ d.content.massage_sales_num}}</a></span></p>
		 </div>
		
		 <div class="wx_products_wtxt2 mt30">
			 {{# if(d.activityId>0){ }}
<p class="a1 fl "><span class="fz30">¥</span><span class="fz40">{{d.discountedMoney}}</span><del class="fz14 txthui01 ml10">¥{{d.content.massage_price}}</del></p>
				
            {{# }else { }}
				<p class="a1 fl "><span class="fz30">¥</span><span class="fz40">{{ d.content.massage_price }}</span></p><!-- {{(Number(d.content.massage_price)* Number(d.discount)/10).toFixed(2) }} -->
            {{# } }}
   ...
   {{# if(d.activityId && d.activityId>0 ){ }}
		<div class="shopckdd_dat shopckdd_datmun">
			...
			<p style="position: relative;top: 9px;">{{d.activityInfo ? d.activityInfo.describe : " "}}</p>
		</div>
   {{# } }}
...
<script type="text/html" id="massageCommentTemp">
		<div class="shopckdd_dat shopckdd_datmun">
				...
				<p class="mb0">{{ d.massageInfo.massage_introduction}}</p>
	    </div>
            {{# if( d.content) { }}
						...
						<p class="cpyy_tatle">用户评价</p>
                         {{# for(var i=0;i<d.content.length;i++) { }}
                           {{# var massageItem = d.content[i]}}
								...
								<p class="fz24 mt20">{{( massageItem.comm_text)?  massageItem.comm_text : "服务很好，值得推荐！"}}</p>
...
<!-- 外部js资源文件引入 -->
<script src="<%=request.getContextPath()%>/resources/javascript/manager/shop/massage/wx_massage_sale_info.js"></script>
