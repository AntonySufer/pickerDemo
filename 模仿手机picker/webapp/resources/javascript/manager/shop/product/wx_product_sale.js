$(function(){
    ...
    var productSale={
        init:function(){
            ...
            //获取customId;
            var	customInfo= globalUtil.getCustomerId(openId,shopId,appId,userId);
            if(customInfo){
        ...
        getProductAccessCard:function(){
							...
							//活动
							 ...
							 if(_date.getTime()>=activLimitMinSec){
								...
								// var activTimeZone =(productInfo.end_time.split(" ")[0]).replace(new RegExp("-","gm"),".");
								 var activTimeZone = new Date(productInfo.end_time).format("yyyy-MM-dd hh:mm");
								 layer.open({
            ...
            jQuery.ajax({
                ...
                data:{ shop_id : shopId,rows:50,page :1},
                type:"get",
                dataType:"json",
                success : function(data){
                    if(data.status == 200 && data.content){
                ...
                var fromItemProcuctApp={
                    ...
                    init:function(){
                        ...
                        jQuery("#makeOrderBtn").click(function(){
                            ...
                            //添加  mainOrderId
                            var mainOrderId=  globalUtil.getMainOrderId(shopId,openId,appId);
                            mainOrderId = globalUtil.b64ToUtf8(mainOrderId, "utf8") ;
                            opt.mainOrderId=mainOrderId;
                        ...
                        if(opt.orderWay==6){
                           ...
                           // alert("预约产品optstring:"+opt);
                            ...
                            /* store.remove("opt");
                             store.set("opt",opt);
                             var state ="";*/
                           // alert("GLOBAL.appIdPay:"+GLOBAL.appIdPay);
