	...
	var shopInfo={
		init:function(){
			 			...
			 			if(data.status==200 && data.content){
		 					...
		 					laytpl(jQuery("#chooseShopTemp").html()).render({
		 						content:shopList
		 					        ...
		 					        var preferShopId = shopList.prefer_id;
		 					        var proferShopName ;
		 					        for(var k=0;k<shopList.shop_list.length;k++){
		 					        	var shopInfoObj =shopList.shop_list[k];
		 					        	for(var att in shopInfoObj){
		 					        		if(att =="shop_id"){
		 					        			if(shopInfoObj["shop_id"]==preferShopId){
		 					        				proferShopName=shopInfoObj["shop_name"]
		 					        			}
		 					        		}
		 					        	}
		 					        }
		 					      ...
		 					      //  var fontNum =shopList.prefer_name.length;
		 					       var fontNum =proferShopName.length;
		 					        var inputUnit =40;
		 					        var divUnit =50;
			...
			if(shopId){
				...
				var customContent= globalUtil.getCustomerId(openId,shopId,appId,userId);
 		        	 ...
 		        	 async: "false",
 		 			 ...
 		 			 success:function(data){
 		 				 if(data.status == 200 && data.content){
 		 					...
 		 					_this._endFun(shopId,shopInfoData.s_id);
