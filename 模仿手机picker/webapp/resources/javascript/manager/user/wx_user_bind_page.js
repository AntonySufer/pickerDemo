			...
			/*	if(status == 28 ){
				...
				$("body").on("keyup","input[name=username]",function(){
					...
					if( userNameLen>0  && ( userNameLen<2 || userNameLen >12) ){
						$(this).after($("<span class='error_name' style='display:inline-block;font-size:24px;position:absolute;width:200px;z-index:999;left:390px;padding-top:25px;color:red;'>输入2-12个字符</span>"))
					}else{
			  ...
			  //获取验证
				...
				//提交绑定
			     $("body").delegate("#submit_btn","click",function(){	 	 
					 ...
					 if(mobile_phone && user_name && verifyNum && shopId && user_name.length>1){
						$.ajax({
