...
$(function(){
	var status=globalUtil.getUrlParam('status') || $("body").attr("data-status"),
			...
			init:function(){
				...
				if(status==29){
					...
					if(shopId){
						...
						//跳转选择店铺页面
		 				location.href = GLOBAL.contextPath + "agent/manager-user-wx_select_shop_for_bind";
					}
				} 
				...
				$("body").on("keyup","input[name=username]",function(){
					...
					if( userNameLen>0  && ( userNameLen<2 || userNameLen >12) ){
						$(this).after($("<span class='error_name' style='display:inline-block;font-size:24px;margin-right:30px;float:right;color:red;'>输入2-12个字符</span>"))
					}else{
			  ...
			  //获取验证
				...
				//提交绑定
			     $("body").delegate("#submit_btn","click",function(){	 	 
					 ...
					 if(mobile_phone && user_name && verifyNum && shopId && user_name.length>2){
						$.ajax({
